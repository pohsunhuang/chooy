...
const TopicsPagination = ({ offset=0, totalCount=0, itemsPerPage=10, query }) => {
  const pageNumbers = [];
  ...
  return (
    <div className={hidden ? 'hidden' : 'topics-pagination'}>
      <ul className='pagination-list'>
        {activePageNumber == 1 ? null:
          <li className='pagination-list-control'>
            <Link to={`/search?query=${query}&offset=${(activePageNumber-2)*(itemsPerPage || 10)}`}>
        ...
        {pageNumbers.map((pageNumber, idx) => {
          if (pageNumber == activePageNumber) {
            ...
            return <li className='pagination-list-item'key={idx}><Link to={`/search?query=${query}&offset=${(pageNumber-1)*(itemsPerPage || 10)}`}>{pageNumber}</Link></li>
          }
        })}
        {activePageNumber == totalPageNumber ? null: 
          <li className='pagination-list-control'>
            <Link to={`/search?query=${query}&offset=${(activePageNumber)*(itemsPerPage || 10)}`}>
...
registerComponent('TopicsPagination', TopicsPagination);
